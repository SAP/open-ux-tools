...
4. Check `empty circle` (UnSaved) indicator is visible for the property `text` in the Properties Panel
5. Hover property `Text` to open tooltip in the Properties Panel
6. Verify tooltip content 
```json
{
  "title": "Text",
  "Property name": "text",
...
7. Find and click on `Create New` node in the Outline Panel
8. Check `Create New` node has `empty circle (UnSaved)` indicator in the Outline Panel
...
5. Check `filled circle` (Saved) indicator is visible for the property `text` in the Properties Panel
...
9. Check `Create New` node has `filled circle (Saved)` indicator in the Outline Panel
10. Fill `Text` property with  `Create Newest` in the Properties Panel
11. Check `half-filled circle` (SavedAndUnSaved) indicator is visible for the property `text` in the Properties Panel
12. Find and click on `Create Newest` node in the Outline Panel
13. Check `Create Newest` node has `half-filled circle (SavedAndUnSaved)` indicator in the Outline Panel
...
3. Fill `Text` property with  `Create New` in the Properties Panel
4. Click `Save` button in the toolBar
5. Check saved changes stack contains `1` `Button` change(s)
6. Confirm there are 1 files in the workspace
7. Hover property `Text` to open tooltip in the Properties Panel
8. Click `Delete all changes for this property` button in the ``text` Property tooltip`
9. Click `Delete` button in the `Confirm property change deletion`
10. Confirm there are no files in the workspace
