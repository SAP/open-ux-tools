...
| Oversized generated result | `packages/mockserver-data-generator/test/unit/api.test.ts`; `packages/mockserver-data-generator-cap/test/plugin.test.ts` | A complete result above 64 MiB UTF-8 is rejected before cache or CAP publication; the standard FE host fallback and normal CAP startup/data remain active with stable `GENERATED_RESULT_TOO_LARGE` reporting |
