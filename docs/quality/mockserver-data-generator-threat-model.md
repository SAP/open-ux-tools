...
- `SAP/open-ux-tools` runtime package commit
  `ffdb7d6a425616a930c522c6e72fecc1938b373c`
...
3. Downloaded model artifacts are data only. The separately identified native
   runtime is executable, but only the current OS/CPU entry declared by the
   package-owned immutable manifest may be downloaded, and it is size- and
   checksum-verified as a complete file set before loading.
...
| Model manifest to local cache | model and platform-runtime URLs, paths, sizes, hashes, and runtime contract | MockGen manifest/downloader | validate immutable identity, supported target, and HTTPS policy before bounded acquisition |
| Local cache to model runtime | ONNX, tokenizer, config, classifier head, and the selected native-runtime file set | verified cache and fixed runtime adapters | reject missing, linked, wrong-sized, or checksum-mismatched files; expose no runtime entry until its complete set verifies; never load model-side code |
...
| T06 | A model bundle executes downloaded code or selects an arbitrary runtime | Model components remain data-only and cannot name entrypoints, hooks, or scripts. Format-2 runtime entries are a separate executable artifact class, restricted to the exact package-owned version and supported OS/CPU target; every declared file is bounded and hashed, no archive or install hook executes, and no application-installed fallback is accepted. The source release builder accepts only a supported current target, exact matching runtime packages, immutable credential-free HTTPS artifact/SBOM URLs, a new absolute output directory, and regular non-linked source files; it emits the manifest descriptor but does not upload or publish it | partial | Execute the documented builder on all targets, approve the hosted file sets, and complete their license, SBOM, provenance, signing, and rollback disposition |
...
| T13 | A vulnerable transitive dependency or native runtime compromises the process | Runtime identity/version is pinned; the all-platform package is absent from consumer dependencies; only the selected verified runtime is loaded; package and runtime footprints are separate; current workspace audit is recorded below | partial | Resolve or formally disposition upstream advisories; produce runtime SBOM/provenance and apply release signing policy |
