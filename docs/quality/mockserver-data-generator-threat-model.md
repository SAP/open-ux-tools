...
| T06 | A model bundle executes downloaded code or selects an arbitrary runtime | Model components remain data-only and cannot name entrypoints, hooks, or scripts. Format-2 runtime entries are a separate executable artifact class, restricted to the exact package-owned version and supported OS/CPU target; every declared file is bounded and hashed, no archive or install hook executes, and no application-installed fallback is accepted | partial | Approve hosted platform file sets and complete their license, SBOM, provenance, signing, and rollback disposition |
