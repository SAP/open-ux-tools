...
- development kit source: `40393c7c6f98d13ad79301b83be7ab24a84a7e46`
...
- portable development-kit fingerprint:
  `3a16a758a6e582080b7769dc34c884dab85f5dad00dfc17ff3b80459e392b774`
- portable development-kit SHA-256:
  `cb2a72cc2973631cda6d2f00b2bf810b634c5d720750b816d932618ec7281db1`
...
1. The 266,364,774-byte passing runtime is an experimental `darwin-arm64`
...
| Generated-data cache | proven locally / platform | fingerprinting, validation, corruption quarantine, atomic writes, concurrent publication, multi-completion SFT statistics, deterministic 32 MiB LRU quota, and 19.072 ms fresh-process p95 without model initialization | Repeat on release platforms |
...
| Total installed/cache footprint | proven locally / distribution pending | exact cache-fix evaluation- and integration-bound `darwin-arm64` footprint: 266,364,774 bytes against a 314,572,800-byte ceiling; upstream multi-platform closure remains 449,503,668 bytes | Convert the proof into supported upstream or SAP-governed platform packages and qualify every release platform |
| Integrated performance | proven locally / platform | current five-sample p95: 2,595.700 ms cold service, 19.072 ms warm cache, 1,221.860 ms first acquisition, and 2,596.510 ms host; peak RSS 1,448,493,056 bytes | Process-tree RSS and the supported Node/OS/BAS platform matrix |
...
| `@sap-ux/mockserver-data-generator` | 23 suites, 198 tests passed; 85.62% statement coverage; build and package check passed; lint has zero errors |
...
| current cache-fix learned V4 archive canary | exact `3a16a758a6e58208` archive verified classifier and SFT, provider execution, metadata, one returned row, and exact restore; the five-sample rerun proved real SFT result publication and warm-cache reuse without model initialization |
...
| current source package archive | 82,508 / 5,242,880 bytes, pass |
...
| experimental platform-runtime total installed and cache | 266,364,774 / 314,572,800 bytes, pass |
| integrated Fiori p95 | 2,595.700 ms cold; 19.072 ms cache; 1,221.860 ms acquisition; 2,596.510 ms host; all pass |
...
266,364,774 bytes—48,208,026 bytes below the ceiling. The footprint report,
