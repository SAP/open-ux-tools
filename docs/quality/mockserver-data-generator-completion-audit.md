...
- reviewed generator, `--mockgen` launcher, zero-setup generated-application
  wiring, deterministic-mode correction, and release hardening:
  `04390772f75852440444a9ec076ee380e61aeaaf`
...
| Local/BAS development kit | reproducible / BAS pending | The current 575,373-byte clean archive is marked reproducible and binds both pushed branch commits. It passed separate standard, deterministic MockGen, and retained-classifier/SFT V4 canaries locally on Node 22.22.2, followed by byte-exact restore. The compatibility archive before it supplies fresh V2/V4/CDS coverage | Run the current fingerprinted archive in BAS |
...
| Realism | measured fail | the exact package candidate has a blinded, randomized 311-record packet covering six domains and EDMX V2/V4/CSN; OpenAI and Google reviewed every field with valid lineage. Pessimistic consensus is 184/311 (59.16%), with one critical issue and 93 disagreements, so the gate correctly fails. Evidence fingerprint `42922a81940f0dcc6fdacdd7648f6d5686238fa65d47623dca89221cc1d82cd7` is bound to package commit `04390772f75852440444a9ec076ee380e61aeaaf` | Correct the ranked semantic/coherence defects, export a new clean candidate, and repeat the frozen two-provider gate; require at least 80% overall and in every domain/format with no critical issue |
...
| current realism cohort | 311 records; 124/124 parsed; 446/462 accepted fields; all 6 targets contribute; 6/6 structural targets and 11/11 frozen assertions passed; evidence and campaign replay byte-identical. Candidate fingerprint: `57f7b96c5a55ae538f47600f688594f954357e651650c5278ac06478434b7dd1`; evidence file SHA-256: `b12911c5bf8b19d5ea69e3de8e575e4121e62892f34adef118a72a85f7764d95` |
| deterministic semantic triage | zero high- or medium-severity signals in the unchanged current packet; the later two-provider result demonstrates that this screen was insufficient for realism |
| current two-provider realism | OpenAI accepted 186/311 (59.81%); Google accepted 252/311 (81.03%); pessimistic consensus accepted 184/311 (59.16%), with one critical issue and 93 disagreements. The gate fails and no production-realism claim is made |
...
| current clean dev kit | 575,373 bytes; fingerprint `9e22ef712db89875ddc5e0b3547654a2db0f3207865028dae61db8e7ba20a3c2`; SHA-256 `a8a5041f94e63719f999dc780acfac0a6b34e949062c69f056265cfebdecc449`; manifest `reproducible: true`; source commits match both pushed feature branches |
| packed application canaries | The current exact archive passed separate standard, deterministic MockGen, and retained-classifier/SFT OData V4 canaries on macOS arm64 Node 22.22.2. Standard mode returned one row without provider execution; flagged mode executed MockGen, and the learned run reported `modelVerified: true` and `learnedRuntimeVerified: true`. Transactional restore returned the fixture byte-for-byte outside disposable `node_modules`. The preceding compatibility archive remains the fresh V2/V4/CDS evidence. The current archive is still only the pending BAS candidate and is not relabelled as a BAS pass |
