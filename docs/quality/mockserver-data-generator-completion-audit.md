...
- development kit source: `4d620e0fd80814f729ac890597626596c8f93dda`
...
1. The 266,367,320-byte passing runtime is an experimental `darwin-arm64`
...
| Generated-data cache | proven locally / platform | fingerprinting, validation, corruption quarantine, atomic writes, concurrent publication, multi-completion SFT statistics, deterministic 32 MiB LRU quota, and 17.611 ms fresh-process p95 without model initialization | Repeat on release platforms |
...
| Total installed/cache footprint | proven locally / distribution pending | exact lint-safety evaluation- and integration-bound `darwin-arm64` footprint: 266,367,320 bytes against a 314,572,800-byte ceiling; upstream multi-platform closure remains 449,503,668 bytes | Convert the proof into supported upstream or SAP-governed platform packages and qualify every release platform |
| Integrated performance | proven locally / platform | current five-sample p95: 2,715.682 ms cold service, 17.611 ms warm cache, 1,001.387 ms first acquisition, and 2,716.444 ms host; peak RSS 1,435,779,072 bytes | Process-tree RSS and the supported Node/OS/BAS platform matrix |
...
| `@sap-ux/mockserver-data-generator` | 23 suites, 198 tests passed; 85.27% statement coverage; build and package check passed; lint has zero errors and production return types, unsafe assignments, default paths, consistent returns, and non-null assertions are enforced |
...
| current production lint-safety learned V4 archive canary | exact `e72683c87dc4ac91` archive verified classifier and SFT, provider execution, metadata, one returned row, reproducible packaging, and exact restore; the production safety rules are enforced by package lint |
...
| current source package archive | 83,011 / 5,242,880 bytes, pass |
...
| experimental platform-runtime total installed and cache | 266,367,320 / 314,572,800 bytes, pass |
| integrated Fiori p95 | 2,715.682 ms cold; 17.611 ms cache; 1,001.387 ms acquisition; 2,716.444 ms host; all pass |
...
266,367,320 bytes—48,205,480 bytes below the ceiling. The footprint report,
