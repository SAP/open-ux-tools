...
| packed application canaries | The current exact archive passed separate standard, deterministic MockGen, and retained-classifier/SFT OData V4 canaries on macOS arm64 Node 22.22.2 and Docker Linux x64 Node 22.23.2; each transactional restore returned the fixture byte-for-byte outside disposable `node_modules`. The preceding compatibility archive remains the fresh V2/V4/CDS evidence. The current archive is still only the pending BAS candidate and is not relabelled as a BAS pass |
