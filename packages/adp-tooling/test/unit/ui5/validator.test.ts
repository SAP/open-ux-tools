...
describe('validateUI5VersionExists', () => {
    ...
    it('should return the not reachable error message for a snapshot version when fetch fails', async () => {
        ...
        const result = await validateUI5VersionExists(version);
        expect(result).toBe(
            'The URL of the SAPUI5 version you have selected is not reachable. The https://sapcdn.com/ui5/1.120.0-snapshot URL must be made accessible through cloud connector and the destination configuration so it can be consumed within the SAPUI5 adaptation project and its SAPUI5 Adaptation Editor.'
        );
    });
