...
import { t } from '../../i18n';
import { createServiceByTags, getServiceInstanceKeys } from '../services/api';
...
export async function getHtml5RepoCredentials(spaceGuid: string, logger: ToolsLogger): Promise<ServiceKeys> {
    try {
        ...
        if (!serviceKeys?.credentials?.length) {
            await createServiceByTags(spaceGuid, 'app-runtime', HTML5_APPS_REPO_RUNTIME, ['html5-apps-repo-rt'], {
                logger
            });
