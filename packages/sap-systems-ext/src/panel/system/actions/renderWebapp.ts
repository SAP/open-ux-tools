...
export async function renderWebApp(context: PanelContext): Promise<void> {
    ...
    if (panelViewType === SystemPanelViewType.Create) {
        await postMessage(createNewSystem(systemInfo));
        return;
    }
...
function getSystemInfo(backendSystem?: BackendSystem): BackendSystem | undefined {
    ...
    if (backendSystem) {
        systemInfo = {
            ...backendSystem,
            systemType: backendSystem.systemType ?? getBackendSystemType(backendSystem) ?? 'OnPrem'
        }; // fallback to OnPrem so that system info can be displayed
    }
    return systemInfo;
}
