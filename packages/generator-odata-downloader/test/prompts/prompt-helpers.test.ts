...
describe('Test getData', () => {
    ...
    test('should run secondary hierarchy query and merge when companion entity exists in entitySetsFlat', async () => {
        const hierarchyProps = { NodeId: 'node-1', DistanceFromRoot: 0 };
        const mainEntityData = [
            {
                PurchaseOrder: 'PO1',
                _PurchaseOrderItem: [{ PurchaseOrder: 'PO1', PurchaseOrderItem: '10', __HierarchyProps: null }]
            }
        ];
        const secondaryEntityData = [
            { PurchaseOrder: 'PO1', PurchaseOrderItem: '10', __HierarchyProps: hierarchyProps }
        ];
        jest.spyOn(odataQueryModule, 'fetchData')
            .mockResolvedValueOnce({ odataResult: { entityData: mainEntityData } } as any)
            .mockResolvedValueOnce({ odataResult: { entityData: secondaryEntityData } } as any);
            ...
            },
            {
                ...
                referencedEntities: {
                    listEntity: {
                        ...
                        semanticKeys: [{ name: 'PurchaseOrder', type: 'Edm.String', value: 'PO1' }]
                    },
                    hierarchyEntities: [
                        {
                            entitySetName: 'PPS_PurchaseOrderItem',
                            qualifier: 'I_HNRltn',
                            nodeProperty: '__HierarchyProps/NodeId',
                            parentProperty: 'P',
                            parentPropertyType: 'Edm.String',
                            isDraft: false,
                            entityTypeKeys: ['PurchaseOrder', 'PurchaseOrderItem', 'IsActiveEntity']
                        },
                        {
                ...
                // PPS_PurOrdItemHierarchy is also in entitySetsFlat (nav prop of PPS_PurchaseOrderItem)
                relatedEntityChoices: {
                    entitySetsFlat: {
                        '_PurchaseOrderItem': 'PPS_PurchaseOrderItem',
                        '_PurchaseOrderItem/_PurOrdItemParentToChild': 'PPS_PurOrdItemHierarchy'
                    },
                    choices: []
                }
            } as unknown as AppConfig,
        ...
        expect(odataQueryModule.fetchData).toHaveBeenCalledTimes(2);
        const secondaryCall = (odataQueryModule.fetchData as jest.Mock).mock.calls[1];
        expect(secondaryCall[0].entitySetName).toBe('PPS_PurOrdItemHierarchy');
        // IsActiveEntity filtered out — not in source entityTypeKeys
        expect(secondaryCall[0].semanticKeys).toEqual([{ name: 'PurchaseOrder', type: 'Edm.String', value: 'PO1' }]);
    ...
    test('should skip secondary hierarchy query when target is not in entitySetsFlat', async () => {
        const mainEntityData = [{ PurchaseOrder: 'PO1' }];
        jest.spyOn(odataQueryModule, 'fetchData').mockResolvedValueOnce({
            odataResult: { entityData: mainEntityData }
        } as any);
        ...
        const result = await getData(
            ...
            },
            {
                appAccess: {},
                referencedEntities: {
                    listEntity: { entitySetName: 'PPS_PurchaseOrder', semanticKeys: [] },
                    hierarchyEntities: [
                        {
                            entitySetName: 'PPS_PurchaseOrderItem',
                            qualifier: 'I_HNRltn',
                ...
                relatedEntityChoices: { entitySetsFlat: {}, choices: [] }
            } as unknown as AppConfig,
            []
        );

        expect(odataQueryModule.fetchData).toHaveBeenCalledTimes(1);
        expect(result).toEqual({ odataQueryResult: mainEntityData });
    });
});
