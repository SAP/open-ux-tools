...
import { promptNames, type InquirerAdapter, type UI5ApplicationAnswers, type UI5ApplicationPromptOptions } from './types';
import autocomplete from 'inquirer-autocomplete-prompt';
import { getCapProjectType, findProjectRoot } from '@sap-ux/project-access';
 ...
 * @param [isCli=true] Changes some prompt logic for YUI execution otherwise prompts will be executed serially in a command line environment
 * @returns the prompts used to provide input for ui5 library generation
 */
async function getPrompts(promptOptions?: UI5ApplicationPromptOptions, isCli = true): Promise<Question<UI5ApplicationAnswers>[]> {
    ...
    if (promptOptions?.targetFolder?.value) {
        // Is this a cap project, target dir will be set to the `app` or custom app folder not the project root
        isCapProject = !!(await getCapProjectType(await findProjectRoot(promptOptions.targetFolder?.value, false, true)));
    }
   /*  const ui5LibPromptInputs: UI5ApplicationPromptOptions = {
    ...
    return getQuestions(ui5Versions, promptOptions, isCli, isCapProject);
}
 ...
 */
async function prompt(promptOptions: UI5ApplicationPromptOptions, adapter: InquirerAdapter): Promise<UI5ApplicationAnswers> {
...
export { getPrompts, prompt, type UI5ApplicationPromptOptions, type UI5ApplicationAnswers };
