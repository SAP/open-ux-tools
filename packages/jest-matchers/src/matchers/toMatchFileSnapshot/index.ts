...
import type { MatcherIgnore } from 'matchers/types';
        ...
        if (replaceGrp.filenames.includes(basename(fileName))) {
            replaceGrp.ignore.forEach((replaceRegEx) => {
                try {
                    ...
                    throw new Error(`Invalid ignore regex provided to file sanpshot matcher: ${replaceRegEx}`);
                }
            });
        }
    });
...
export function toMatchFile(
    ...
    const { isNot, snapshotState, utils } = this as jest.MatcherContext;
