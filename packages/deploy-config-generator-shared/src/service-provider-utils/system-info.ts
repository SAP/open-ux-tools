...
export async function getSystemInfo(
    ...
    try {
        const provider = await getOrCreateServiceProvider(systemConfig, logger, backendTarget);
        const lrep = provider.getLayeredRepository();

        return lrep.getSystemInfo(packageName);
    } catch (e) {
        logger.debug(t('errors.debugAbapTargetSystem', { method: 'getSystemInfo', error: e.message }));
        return undefined;
    }
}
