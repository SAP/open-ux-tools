...
describe('Test service url validators', () => {
    ...
    test('should validate service metadata', async () => {
        ...
        const odataService = createServiceForUrl('https://some.host:1234/service/path');
        jest.spyOn(odataService, 'metadata').mockResolvedValueOnce(invalidMetadata);
        expect(await validateService(odataService)).toMatch(t('prompts.validationMessages.metadataInvalid'));

        // Valid metadata
        jest.spyOn(odataService, 'metadata').mockResolvedValue(validMetadata);
        expect(await validateService(odataService)).toBe(true);

        // Valid metadata with required version
        expect(await validateService(odataService, OdataVersion.v4)).toBe(
        ...
        expect(await validateService(odataService, OdataVersion.v2)).toBe(true);
    });
    ...
    test('should reset prompt state and log if an error occurs', async () => {
        ...
        expect(await validateService(odataService)).toBe(
            t('errors.unknownError', { error: metadataRequestError.message })
        );
        ...
        expect(await validateService(odataService)).toBe(t('errors.odataServiceUrlNotFound'));
    });
});
