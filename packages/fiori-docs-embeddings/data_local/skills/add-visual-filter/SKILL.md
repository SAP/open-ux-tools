...
argument-hint: 'field name to add visual filter to (e.g., "Destination", "Status")'
---
...
service YourService {
  @Aggregation.ApplySupported: {
    ...
    AggregatableProperties: [
      { Property: MeasureField }  // e.g., TotalPrice, Amount, Quantity
    ],
    GroupableProperties: [
      DimensionField1,  // e.g., Status, Destination, Category
      DimensionField2,
...
annotate service.YourEntity @(
    UI.Chart #visualFilter : {
        ...
        Dimensions : [
            DimensionField  // Field to group by (e.g., Destination, Status)
        ],
        DynamicMeasures : [
            '@Analytics.AggregatedProperty#MeasureField_sum'
        ]
...
Based on the travel-management project implementation:
...
service TravelService {
  ...
  entity Travels as projection on travel.Travels;
}
```
...
**Manifest Configuration (app/travels/webapp/manifest.json):**
```json
{
  "sap.ui5": {
    "routing": {
...
2. Open the Fiori application
3. Click on the filter field (e.g., Destination) in the filter bar
4. The value help dialog should display a bar chart showing aggregated data
5. Click on a bar to filter by that value
