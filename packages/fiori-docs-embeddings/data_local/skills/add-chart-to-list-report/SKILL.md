---
name: add-chart-to-list-report
description: 'Add analytical charts to SAP Fiori List Report pages with aggregation support. Use for: visualizing aggregated data, adding column/bar/line/donut charts, configuring average/sum/min/max measures, enabling chart/table hybrid views, implementing analytics in Fiori Elements OData V4 apps.'
argument-hint: 'Specify entity name, dimension field, measure field, and chart type (e.g., "Travels by Destination with average price as column chart")'
---
...
**Action:** Explicitly ask the user to provide the following details:
- **Target entity name** - Which entity should have the chart? (e.g., Travels, Products, Orders)
- **Dimension field** - Field to group data by (e.g., Destination, Category, Status)
- **Measure field** - Numeric field to aggregate (e.g., TotalPrice, Revenue, Quantity)
- **Aggregation method** - How to calculate: average, sum, min, max, or countdistinct
- **Chart type** - Preferred visualization: Column, Bar, Line, Donut, Area, or Bubble

If not provided, prompt the user with clarifying questions before proceeding.
...
**Note:** Replace `ServiceName`, `EntityName`, `MeasureField`, `DimensionField` with actual names from your project.
