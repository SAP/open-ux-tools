 ...
 */
export async function generate(cfWriterConfig: CFAppConfig, fs?: Editor, logger?: Logger): Promise<Editor> {
    if (!fs) {
    ...
    const cfConfig = await getCFConfig(cfWriterConfig, fs);
...
async function getCFConfig(cfAppConfig: CFAppConfig, fs: Editor): Promise<CFConfig> {
    let isCap = false;
    let mtaPath;
    let mtaId;
    let rootPath;
    ...
    if (!cfConfig.mtaId) {
        try {
            if (cfConfig.isCap) {
                ...
                await createMTA(cfConfig as MTABaseConfig, fs);
            }
            logger?.debug(`MTA Created at ${join(cfConfig.mtaPath, MTAYamlFile)}`);
        } catch (error) {
...
async function generateRootConfig(cfConfig: CFConfig, fs: Editor, logger?: Logger): Promise<void> {
    try {
        ...
        if (cfConfig.addManagedApprouter && !fs.exists(join(cfConfig.rootPath, 'xs-security.json'))) {
            fs.copyTpl(getTemplatePath(`common/${XSSecurityFile}`), join(cfConfig.rootPath, XSSecurityFile), {
...
async function generateAppConfig(cfConfig: CFConfig, fs: Editor, logger?: Logger): Promise<void> {
    try {
        ...
        await generateUI5DeployConfig(cfConfig.appPath, cfConfig.appId, fs);
    } catch (error) {
 ...
 */
export async function generateUI5DeployConfig(appPath: string, appId: string, fs: Editor): Promise<void> {
    const ui5BaseConfig = await readUi5Yaml(appPath, FileName.Ui5Yaml, fs);
    ...
    ui5DeployConfig.addCloudFoundryDeployTask(appId, addModulesTask, addTranspileTask);
    fs.write(join(appPath, FileName.UI5DeployYaml), ui5DeployConfig.toString());
}
