...
async function createDeployService(config: AbapDeployConfig, logger?: Logger): Promise<Ui5AbapRepositoryService> {
    ...
    if (isAppStudio() && config.target.destination) {
        ...
        if (config.target.cloud) {
            provider = await createAbapCloudServiceProvider(options, config.target, config.abortRetry, logger);
        } else {
...
async function handleError(
    ...
    if (!config.abortRetry && isAxiosError(error)) {
        ...
        case 412:
            ...
            if (config.yes || (await promptConfirmation('Do you want to overwrite (Y/n)?'))) {
                await deploymentCommands[command](
                    service,
                    { ...config, safe: false, abortRetry: true },
                    logger,
                    archive
                );
                return true;
