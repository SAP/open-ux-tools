 ...
 * @param maxWordPieceTokens
 */
export function createMiniLmTokenizer(vocabularyText: string, maxWordPieceTokens?: number): MiniLmTokenizer {
    if (maxWordPieceTokens !== undefined && (!Number.isSafeInteger(maxWordPieceTokens) || maxWordPieceTokens < 2)) {
        throw new TypeError('MiniLM token budget must include at least two boundary tokens');
    }
    ...
    return Object.freeze({
        encodeForModel: (text: string) => {
            ...
            const bounded = maxWordPieceTokens === undefined ? pieces : pieces.slice(0, maxWordPieceTokens - 2);
            const inputIds = ['[CLS]', ...bounded, '[SEP]'].map((token) => vocabulary.get(token) as number);
            return Object.freeze({
