...
function generateService(...[request, options, runtime]: Parameters<typeof generate>): ReturnType<typeof generate> {
    ...
    return generate(
        request,
        {
            ...options,
            syntheticScenario: {
                id: 'coherence-test-scenario',
                version: '1',
                domains: {},
                coherence: Object.fromEntries(request.targets.map(({ name }) => [name, rules]))
            }
        },
        runtime
    );
...
function expectCoherentDocument(row: Readonly<Record<string, unknown>>): void {
    ...
    expect(STATUS_SAMPLES.find(({ code }) => code === row.StatementStatus)?.text).toBe(row.StatementStatus_Text);
...
function expectCoherentUnit(row: Readonly<Record<string, unknown>>): void {
    const sample = UNIT_SAMPLES.find(({ code }) => code === row.UnitOfMeasure);
    expect(sample?.text).toBe(row.UnitOfMeasure_Text);
    expect(row.UnitOfMeasureISOCode).toBe(sample?.iso);
}
