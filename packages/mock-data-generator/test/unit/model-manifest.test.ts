...
describe('model manifest', () => {
    ...
    test('accepts a v3 classifier runtime without changing the SFT contract', () => {
        const candidate = validManifest() as any;
        candidate.components[0].runtime.outputFormat = 'embedding-classifier-v3';
        const manifest = parseModelManifest(candidate);
        expect(manifest.components[0]?.runtime.outputFormat).toBe('embedding-classifier-v3');
        expect(manifest.components[1]?.runtime.outputFormat).toBe('row-object-v1');
    });
