import { generateService as generate, type SftGenerator } from '../../src/index.js';
import { parseCsn } from '../../src/schema/csn.js';
import { parseEdmx } from '../../src/schema/edmx.js';

// These are explicitly authored fictional test domains, not default generator behavior.
const scenarioDomains: Readonly<Record<string, readonly (string | number | boolean)[]>> = {
    ChartOfAccounts: ['YCOA', 'INT', 'CAUS', 'IFRS'],
    BillableControl: ['01', '02', '03', '04'],
    Plant: ['1010', '1110', '1710', '3010'],
    UniqueItemIdentifierRespPlant: ['1010', '1110', '1710', '3010'],
    DECertifiedDiversityStatus: [0, 1, 2, 3],
    GLAccount: ['0000113100', '0000400000'],
    BankDataStorageApplication: ['EBS', 'MANL', 'API', 'FILE'],
    HouseBank: ['DE01', 'US01', 'GB01', 'IE01'],
    ServiceDocumentItemStatus: ['O', 'I', 'A', 'C'],
    BankAccountType: ['Checking', 'Savings', 'Money Market', 'Current'],
    BankStatementFormat: ['MT'],
    PaymentTransactionTypeGroup: ['INBOUND', 'OUTBOUND', 'TRANSFER'],
    CompanyCode: ['1000', '1010', '1710', '3000'],
    SalesOrganization: ['1000', '1010', '1710', '3000'],
    DistributionChannel: ['10', '20', '30'],
    Division: ['00', '01', '10', '20'],
    OrganizationDivision: ['00', '01', '10', '20'],
    StorageLocation: ['0001', '0002', 'A001', 'B001'],
    PaymentTerms: ['0001', '0002', 'Z030', 'Z060'],
    SalesItemProposalType: ['OR', 'QT', 'SIP'],
    ServiceDocItemCategory: ['SRVP', 'SVCP', 'SVCT'],
    ServiceDocumentType: ['SRVO', 'SVC1', 'SVO1'],
    ObjectType: ['BUS2000116', 'BUS2000120'],
    TechObjIsEquipOrFuncnlLoc: ['Equipment', 'Functional Location'],
    ServiceDocumentItemObjectType: ['BUS2000116', 'BUS2000120'],
    ApprovalStatus: ['Approved', 'Pending', 'Rejected', 'Under Review'],
    DEConfidence: ['High', 'Medium', 'Low'],
    DEBusinessStatus: ['Active', 'Inactive', 'Out of Business', 'Unknown'],
    DECSSClass: [1, 2, 3, 4, 5]
};

function generateService(...[request, options, runtime]: Parameters<typeof generate>): ReturnType<typeof generate> {
    const graph =
        request.metadata.format === 'csn' ? parseCsn(request.metadata.content) : parseEdmx(request.metadata.content);
    const domains = Object.fromEntries(
        graph.entities.flatMap((entity) =>
            entity.properties.flatMap((property) =>
                scenarioDomains[property.name]
                    ? [[`${entity.entitySetName}.${property.name}`, scenarioDomains[property.name]]]
                    : []
            )
        )
    );
    ...
    return generate(
        request,
        {
            ...options,
            syntheticScenario: {
                id: 'authored-regression-scenario',
                version: '1',
                domains,
                coherence: Object.fromEntries(graph.entities.map(({ entitySetName }) => [entitySetName, rules]))
            }
        },
        runtime
    );
...
const ORGANIZATION_PATTERN = alternation(DEFAULT_SAMPLE_DATASET.organizations);
const CITY_PATTERN = alternation(LOCATIONS.map(({ city }) => city));
const MOBILE_PATTERN = new RegExp(
    `^(?:${LOCATIONS.map(
        ({ mobilePrefix, mobileSubscriberDigits }) =>
            `${mobilePrefix.replace('+', '\\+')} \\d{${mobileSubscriberDigits}}`
    ).join('|')})$`,
    'u'
);
...
describe('semantic realism regressions', () => {
    ...
    it('uses a governed organization bank for supplier names before SFT', async () => {
        ...
        expect(result.resources.Supplier?.[0]?.CommonSupplierName).toMatch(ORGANIZATION_PATTERN);
        expect(sft.generate).not.toHaveBeenCalled();
    });
    ...
    it('prefers unambiguous business metadata over a conflicting learned role', async () => {
        ...
        expect(row?.DECity).toMatch(CITY_PATTERN);
        expect(LOCATIONS.map(({ country }) => country)).toContain(row?.DECountry);
        expect(LOCATIONS.map(({ countryName }) => countryName)).toContain(row?.DECountryDescription);
        expect(LOCATIONS.map(({ regionName }) => regionName)).toContain(row?.DERegionDescription);
        ...
        expect(row?.CompanyCode_Text).toMatch(ORGANIZATION_PATTERN);
    });
    ...
    it('defers contradictory lexical evidence to a confident learned classification', async () => {
        ...
        expect(result.resources.Contact?.[0]?.EmailCode).toMatch(CITY_PATTERN);
    });
    ...
    it('uses governed SAP bank-statement values instead of opaque identifiers or generated prose', async () => {
        ...
        rows.forEach((row, rowIndex) => {
            ...
            expect(['DE01', 'US01', 'GB01', 'IE01', 'IT01', 'CZ01']).toContain(row.HouseBank);
            expect(Number(row.NumberOfItems)).toBeGreaterThanOrEqual(1);
            expect(Number(row.NumberOfItems)).toBeLessThanOrEqual(500);
        });
        expect(sft.generate).not.toHaveBeenCalled();
    ...
    it('uses governed SAP transaction and organization codes instead of residual SFT guesses', async () => {
        ...
        result.resources.Transaction?.forEach((row, rowIndex) => {
            ...
            expect(row.BankAccountHolderName).toMatch(ORGANIZATION_PATTERN);
            ...
            expect(DEFAULT_SAMPLE_DATASET.organizations.filter((name) => name.length <= 20)).toContain(
    ...
    it('keeps sales contacts and unit-master values in their SAP business domains', async () => {
        ...
        result.resources.SalesRecords?.forEach((row) => {
            ...
            expect(row.MobilePhoneNumber).toMatch(MOBILE_PATTERN);
    ...
    it('uses equipment names and compact IUID configuration values for maintenance fields', async () => {
        ...
        result.resources.Equipment?.forEach((row) => {
            expect(EQUIPMENT_NAMES).toContain(row.EquipmentName);
            expect(EQUIPMENT_NAMES).toContain(row.TechnicalObjectDescription);
