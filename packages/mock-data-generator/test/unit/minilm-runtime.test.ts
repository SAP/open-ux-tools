...
describe('MiniLM ONNX classifier runtime', () => {
    ...
    test('caps v3 model input at 64 WordPieces including boundary tokens', async () => {
        const vocabularyPath = join(directory, 'vocab.txt');
        await writeFile(vocabularyPath, ['[PAD]', '[UNK]', '[CLS]', '[SEP]', 'email'].join('\n'));
        const run = jest.fn(async (feeds) => {
            expect(feeds.input_ids.dims).toEqual([1, 64]);
            expect(feeds.input_ids.data[63]).toBe(3n);
            return { last_hidden_state: { data: new Float32Array(128).fill(1), dims: [1, 64, 2] } };
        });
        const embedder = await createMiniLmTextEmbedder({
            modelPath: 'unused',
            vocabularyPath,
            ...
            backend: { createSession: async () => ({ run }), tensor: (_type, data, dims) => ({ data, dims }) }
        });
        await embedder.embed(['email '.repeat(300)], new AbortController().signal);
        await embedder.dispose();
    });

    test('rejects mismatched encoder bytes before allocating a native session', async () => {
        const vocabularyPath = join(directory, 'vocab.txt');
        const modelPath = join(directory, 'model.onnx');
        await writeFile(vocabularyPath, '[UNK]\n[CLS]\n[SEP]');
        await writeFile(modelPath, 'wrong encoder');
        const createSession = jest.fn();
        await expect(
            createMiniLmTextEmbedder({
                modelPath,
                vocabularyPath,
                hiddenSize: 2,
                expectedEncoderSha256: 'a'.repeat(64),
                backend: { createSession, tensor: (_type, data, dims) => ({ data, dims }) }
            })
