...
describe('mockserver data generator public API', () => {
    it('inspects the exact generation decisions with raw values only when explicitly requested', async () => {
                        ...
                        <edmx:DataServices>
                            <Schema Namespace="Demo" xmlns="http://docs.oasis-open.org/odata/ns/edm">
                                ...
                                <EntityType Name="Record">
                                    ...
                                    <Property Name="MarketNation" Type="Edm.String" Nullable="false" MaxLength="40" />
                                </EntityType>
                            </Schema>
                        </edmx:DataServices>
                    </edmx:Edmx>`
        ...
        const classifier: SemanticClassifier = {
            fingerprint: 'inspection-classifier-v1',
            classify: async (input) =>
                input.propertyName === 'MarketNation'
                    ? {
                          role: 'country',
                          confidence: 0.94,
                          source: 'classifier',
                          routeThreshold: 0.9,
                          top: [
                              { role: 'country', confidence: 0.94 },
                              { role: 'region', confidence: 0.04 }
                          ]
                      }
                    : {
                          role: 'unknown',
                          confidence: 0.81,
                          source: 'unknown',
                          top: [{ role: 'unknown', confidence: 0.81 }]
                      }
        };
        const generatorApi = await import('../../src/index.js');
        const inspectService = Reflect.get(generatorApi, 'inspectService');

        expect(typeof inspectService).toBe('function');
        ...
        expect(inspection).toMatchObject({
            ...
            hashes: {
                ...
                metadata: expect.stringMatching(/^[a-f0-9]{64}$/)
            },
            sourceOwnership: [
                {
                    resource: 'Records',
                    eligible: true,
                    initialRows: { source: 'none', present: false, rowCount: 0 }
                }
            ],
            fieldDecisions: expect.arrayContaining([
                ...
                expect.objectContaining({
                    ...
                    providerState: 'legacy-unverified'
                })
            ]),
            generatedSummary: [{ resource: 'Records', rowCount: 1, sha256: expect.stringMatching(/^[a-f0-9]{64}$/) }],
            generatedValues: { Records: [expect.objectContaining({ ID: 1 })] },
            invariants: expect.arrayContaining([
                { name: 'generated-result', passed: true },
                { name: 'relationships', passed: true }
            ]),
            metrics: {
                timingsMs: { total: expect.any(Number) },
                rssBytes: { before: expect.any(Number), after: expect.any(Number) }
            }
        });
    });
    ...
    it('fingerprints material inputs and learned components but not abort-signal identity', () => {
        ...
        // Snapshots generated before tuple-domain enforcement must not be reused.
        expect(first).not.toBe('c1eda6c6f03e1054165309ae730f386d17cdbe1ce60eb12380349774501703cb');
    ...
    it('accepts a bounded wide-entity SFT timeout and rejects values above two minutes', async () => {
        ...
        await expect(generateService(request, { sftBudgetMs: 120_000 })).resolves.toBeDefined();
        await expect(generateService(request, { sftBudgetMs: 120_001 })).rejects.toThrow(
            'SFT budget must be an integer between 1 and 120000 milliseconds'
        );
    });
    ...
    it('generates deterministic, type-correct rows from EDMX without a learned runtime', async () => {
        ...
        expect(first.resources.Products).toEqual(
            expect.arrayContaining([
                expect.objectContaining({
                    Name: expect.stringMatching(new RegExp(`^(?:${PRODUCTS.join('|')})$`)),
    ...
    it('opens the classifier circuit after one failure and emits one diagnostic', async () => {
        ...
        expect(result.diagnostics.filter(({ code }) => code === 'CLASSIFIER_INFERENCE_FAILED')).toEqual([
