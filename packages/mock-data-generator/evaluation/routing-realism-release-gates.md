...
> **Superseded in part (2026-09-24, performance plan v2):** a synthetic linked code/text domain without a
> model or relevance verifier no longer aborts. By owner decision generation now keeps its deterministic values
> with an `SFT_CANDIDATE_VERIFIER_UNAVAILABLE` warning, verifies linked texts row by row and resource by
> resource, and reports an unverifiable value-list extension as a conflict. This is a release-policy change for
> the owner to confirm; see `perf-v2-preregistration.md`.
