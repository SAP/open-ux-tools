...
Examples:
    \`npx --yes @sap-ux/create@latest convert eslint-config\``
        ...
        .action(async (path, options) => {
            ...
            await convertEslintConfig(path || process.cwd(), options.config, !!options.skipInstall);
        });
}
 ...
 */
async function convertEslintConfig(basePath: string, config: string, skipInstall = false): Promise<void> {
    const logger = getLogger();
    try {
        logger.debug(`Called add eslint-config for path '${basePath}'`);
