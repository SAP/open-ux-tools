...
import { promptGeneratorInput, generate, AdpWriterConfig } from '@sap-ux/adp-tooling'
...
async function generateAdaptationProject(
    ...
    try {
        logger.debug(
            `Called generate adaptation-project for path '${basePath}', skip install is '${skipInstall}'`
        );
