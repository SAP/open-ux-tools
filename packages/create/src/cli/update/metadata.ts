...
async function updateMetadata(
    ...
    try {
        ...
        const sappManifest = manifest['sap.app'] as unknown as
            { dataSources?: Record<string, { settings?: { odataVersion?: string } }> } | undefined;
        ...
        if (isAppStudio()) {
            if (!backendConfig.destination) {
                logger.error(
                    `No destination configured in '${FileName.Ui5Yaml}'. Cannot connect to backend in SAP Business Application Studio.`
                );
                return;
            }
