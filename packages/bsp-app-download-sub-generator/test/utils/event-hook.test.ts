import { runPostAppGenHook, type BspAppGenContext } from '../../src/utils/event-hook';
...
describe('runPostAppGenHook', () => {
    let mockContext: BspAppGenContext;
    ...
    it('should log an error if vscodeInstance is missing', async () => {
        ...
        expect(BspAppDownloadLogger.logger.error).toBeCalledWith(t('error.eventHookErrors.vscodeInstanceMissing'));
    });

    it('should log an error if postGenCommand is missing', async () => {
        mockContext.postGenCommand = '';
        await runPostAppGenHook(mockContext);
        expect(BspAppDownloadLogger.logger.error).toBeCalledWith(t('error.eventHookErrors.postGenCommandMissing'));
    });
    ...
    it('should log an error if executeCommand throws an error', async () => {
        ...
        await runPostAppGenHook(mockContext);
        expect(BspAppDownloadLogger.logger.error).toBeCalledWith(t('error.eventHookErrors.commandExecutionFailed'));
    });
});