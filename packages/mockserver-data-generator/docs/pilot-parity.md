...
| Classify business meaning before choosing values                                                   | Preserved through CAP, SAP V2, and V4 annotation evidence, conservative lexical routing, and the retained MiniLM embedding classifier | `test/unit/metadata-semantics.test.ts`, `test/unit/embedding-classifier.test.ts`, `test/unit/minilm-runtime.test.ts`, and the governed classifier evaluation |
