...
import { type Package, hasDependency, FileName, addPackageDevDependency } from '@sap-ux/project-access';
...
async function updatePackageJson(basePath: string, fs: Editor, logger?: ToolsLogger): Promise<void> {
    await addPackageDevDependency(basePath, 'eslint', '^9.0.0', fs);
    await addPackageDevDependency(basePath, '@sap-ux/eslint-plugin-fiori-tools', '^9.0.0', fs);
    const packageJsonPath = join(basePath, FileName.Package);
