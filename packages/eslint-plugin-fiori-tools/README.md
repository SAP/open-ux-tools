...
|:---------:|------|-------------|:-----------:|:-----------------------:|
|  new      | [sap-text-arrangement-hidden](docs/rules/sap-text-arrangement-hidden.md) | Ensures that the text property referenced by `UI.TextArrangement` (via `Common.Text`) is not hidden via `UI.Hidden` | | ✅ |
