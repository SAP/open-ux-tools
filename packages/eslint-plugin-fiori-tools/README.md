...
|:---------:|------|-------------|:-----------:|:-----------------------:|
|  new      | [sap-no-live-mode](docs/rules/sap-no-live-mode.md) | Ensures that the live mode is not enabled. | | ✅ |
|  9.13.0   | [sap-description-column-label](docs/rules/sap-description-column-label.md) | Ensures that the description (text) property referenced via `Common.Text` has a meaningful `Common.Label` — not a generic value such as `"Name"` or `"Description"`, and not the same label as the ID property. | | ✅ |
