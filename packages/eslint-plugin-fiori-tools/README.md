...
|:-----:|------|-------------|:-----------:|:-----------------------:|
| new   | [sap-condensed-table-layout](docs/rules/sap-condensed-table-layout.md) | Requires `condensedTableLayout` to be enabled when using a grid table, analytical table, or tree table. | | ✅ |
...
| 9.3.1 | [sap-state-preservation-mode](docs/rules/sap-state-preservation-mode.md) | Ensures a valid `statePreservationMode` configuration in SAP Fiori elements applications. | | ✅ |
| 9.1.0 | [sap-copy-to-clipboard](docs/rules/sap-copy-to-clipboard.md) | Ensures that the copy functionality in tables is enabled. The "Copy" button is shown by default. | | ✅ |
...
| 9.1.0 | [sap-flex-enabled](docs/rules/sap-flex-enabled.md) | Ensures that the `flexEnabled` property is set to `true` in the `sap.ui5` section of the `manifest.json` file for applications using SAPUI5 version 1.56 or higher. | | ✅ |
| 9.1.0 | [sap-width-including-column-header](docs/rules/sap-width-including-column-header.md) | Ensures that small tables (less than six columns) have the `widthIncludingColumnHeader` property set to `true` for improved calculation of the column width. | | ✅ |
...
| 9.0.0 | [sap-no-event-prop](docs/rules/sap-no-event-prop.md) | Detect the usage of private members from `sap.ui.base.Event`. Use `sap-no-ui5base-prop` instead. | | |
...
| 9.0.0 | [sap-no-hardcoded-color](docs/rules/sap-no-hardcoded-color.md) | Detect the usage of hardcoded colors. | ✅ | ✅ |
| 9.0.0 | [sap-no-hardcoded-url](docs/rules/sap-no-hardcoded-url.md) | Detect the usage of hardcoded (non-relative) URLs. | ✅ | ✅ |
...
| 9.0.0 | [sap-no-inner-html-access](docs/rules/sap-no-inner-html-access.md) | Detect the usage of the `innerHTML` property. | ✅ | ✅ |
| 9.0.0 | [sap-no-inner-html-write](docs/rules/sap-no-inner-html-write.md) | Detect overriding of the `innerHTML` property. | ✅ | ✅ |
| 9.0.0 | [sap-no-jquery-device-api](docs/rules/sap-no-jquery-device-api.md) | Detect the usage of the deprecated `jQuery.device` API. | ✅ | ✅ |
...
| 9.0.0 | [sap-no-override-rendering](docs/rules/sap-no-override-rendering.md) | Detect overriding of rendering, getters, or setters for SAPUI5 objects. | ✅ | ✅ |
| 9.0.0 | [sap-no-override-storage-prototype](docs/rules/sap-no-override-storage-prototype.md) | Detect overidding of the storage prototype. | ✅ | ✅ |
...
| 9.0.0 | [sap-no-ui5-prop-warning](docs/rules/sap-no-ui5-prop-warning.md) | Detect the usage of private members of the `sap.ui.model.odata.v2.ODataModel`. | ✅ | ✅ |
| 9.0.0 | [sap-no-ui5base-prop](docs/rules/sap-no-ui5base-prop.md) | Detect the usage of private members from `sap.ui.base` classes. | ✅ | ✅ |
...
| 9.0.0 | [sap-no-ui5odatamodel-prop](docs/rules/sap-no-ui5odatamodel-prop.md) | Detect private property usage of the SAPUI5 OData model. Use `sap-no-ui5base-prop` instead. | | |
| 9.0.0 | [sap-no-window-alert](docs/rules/sap-no-window-alert.md) | Detect the usage of `window.alert`. | | |
...
| 9.0.0 | [sap-ui5-legacy-factories](docs/rules/sap-ui5-legacy-factories.md) | Detect legacy SAPUI5 factories that lead to synchronous loading. | ✅ | ✅ |
| 9.0.0 | [sap-ui5-legacy-jquerysap-usage](docs/rules/sap-ui5-legacy-jquerysap-usage.md) | Detect legacy `jQuery.sap` usage. | ✅ | ✅ |
| 9.0.0 | [sap-ui5-no-private-prop](docs/rules/sap-ui5-no-private-prop.md) | Detect the usage of private properties and functions of SAPUI5 elements. | | |
