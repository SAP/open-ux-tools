...
|:---------:|------|-------------|:-----------:|:-----------------------:|
|  new  | [sap-no-path-hidden-on-interactive-columns](docs/rules/sap-no-path-hidden-on-interactive-columns.md) | `UI.Hidden` with a dynamic path must not be used on sortable or filterable columns. | | ✅ |
