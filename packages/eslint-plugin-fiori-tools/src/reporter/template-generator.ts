/**
 * @file Template generator will create the full HTML template for the reporter based on the results and options passed in.
...
const styles = _.template(fs.readFileSync(path.join(__dirname, 'helpers/styles.html'), 'utf-8')),
    ...
    resultDetailsTemplate = _.template(
        fs.readFileSync(path.join(__dirname, 'templates/details/details.html'), 'utf-8')
    ),
    resultSummaryTemplate = _.template(
        fs.readFileSync(path.join(__dirname, 'templates/details/summary.html'), 'utf-8')
    ),
    codeWrapperTemplate = _.template(
        fs.readFileSync(path.join(__dirname, 'templates/details/code/code-wrapper.html'), 'utf-8')
    ),
    ...
    summaryDetailsTemplate = _.template(
        fs.readFileSync(path.join(__dirname, 'templates/summary/summary-details.html'), 'utf-8')
    ),
    ...
    mostProblemsTemplate = _.template(
        fs.readFileSync(path.join(__dirname, 'templates/summary/most-problems.html'), 'utf-8')
    ),
...
function getRuleLink(ruleId) {
    ...
    } else if (_.startsWith(ruleId, 'sap')) {
        ruleId = ruleId.replace('fiori-custom/', '');
        ruleId = ruleId.replace('fiori-tools/', '');
        ruleLink = `https://github.com/SAP/open-ux-tools/tree/main/packages/eslint-plugin-fiori-tools/docs/rules/${ruleId}.md`;
    }
    return ruleLink;
}
 ...
 * @returns {string} HTML string of all the summary detail tables that are needed
 */
...
function renderResultDetails(sourceCode, messages, parentIndex) {
    ...
    return resultDetailsTemplate({
        ...
        detailSummary: resultSummaryTemplate({
            topIssues: typeof topIssues === 'string' ? '' : renderSummaryDetails(topIssues, [], ''),
...
function getOutputDir() {
    const outputOptionIdx =
            process.argv.indexOf('-o') !== -1 ? process.argv.indexOf('-o') : process.argv.indexOf('--output-file'),
...
export function generateTemplate(results: any[], isMultiOn: boolean): string {
    ...
    return pageTemplate({
        ...
        scripts:
            isMultiOn && isOutputDirKnown() ? '<script type="text/javascript" src="./main.js"></script>' : scripts()
    });
}
