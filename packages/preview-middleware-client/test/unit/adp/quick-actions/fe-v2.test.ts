    ...
    describe('ListReport', () => {
        ...
        describe('Enable Semantic Date Range', () => {
            const testCases: {
                ...
                versionInfo: string;
                isManifestPageSettingInArrayStructure: boolean;
            }[] = [
                {
                    validVersion: true,
                    versionInfo: '1.96.37',
                    isManifestPageSettingInArrayStructure: false
                },
                {
                    validVersion: true,
                ...
                },
                {
                    validVersion: true,
                    versionInfo: '1.130',
                    isManifestPageSettingInArrayStructure: true
                }
            ];
            test.each(testCases)('initialize and execute action (%s)', async (testCase) => {
                ...
                const rtaMock = new RuntimeAuthoringMock({} as RTAOptions) as unknown as RuntimeAuthoring;
                const pages = testCase.isManifestPageSettingInArrayStructure
                    ? [{ name: 'test', id: 'test' }]
                    : { name: 'test', id: 'test' };
                    ...
                    quickActionListChanged([
                        {
                            title: 'LIST REPORT',
                            actions:
                                testCase.validVersion && !testCase.isManifestPageSettingInArrayStructure
                                    ? [
                                          {
                                              'kind': 'simple',
                ...
                if (testCase.validVersion && !testCase.isManifestPageSettingInArrayStructure) {
                    await subscribeMock.mock.calls[0][0](
