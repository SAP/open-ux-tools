				...
				html.replace(rxhtmlTag, function ($0, $1, $2) {
					if (rNonVoidHtml5Tags.test($2) && $0.length < html.length) {
						...
						var errorMessage =
							...
							"HTML element used as self-closing tag: <" +
							$1 +
							"/>\n" +
							"HTML element should be closed correctly, such as: " +
							replacement +
							"\n" +
						...
						try {
							sap.ui.require(
								["sap/m/MessageBox", "sap/m/FormattedText", "sap/base/security/encodeXML"],
								function (MessageBox, FormattedText, encodeXML) {
									var messageText = new FormattedText({
										htmlText:
											encodeXML(errorMessage).replace(/&#xa;/g, "<br>") +
											'<a href="' +
											noteUrl +
		...
		sap.ui.define(["sap/ui/thirdparty/jquery-compat"], (jQueryCompat) => {
			const jQuery = (function (global, factory) {
				...
				var isFunction = function isFunction(obj) {
					...
					// (e.g., `typeof document.getElementsByTagName("div") === "function"`). (gh-4756)
					return (
						typeof obj === "function" && typeof obj.nodeType !== "number" && typeof obj.item !== "function"
					);
				};
								...
								try {
									sap.ui.require(
										...
										function () {
											/* eslint-disable no-console, no-alert */
											console.error(
												"Showing error with UI5 controls failed. Falling back to alert()."
											);
											setTimeout(function () {
												alert(errorMessageWithUrl);
											});
