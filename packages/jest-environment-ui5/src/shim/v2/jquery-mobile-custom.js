...
sap.ui.define(["sap/ui/thirdparty/jquery", "sap/ui/Device"], (jQuery) => {
	...
	(function (root, jQuery, doc, factory) {
	...
	})(window, jQuery, document, function (jQuery, window, document, undefined) {
		...
		(function ($, undefined) {
			...
			if (
				...
				!(sap.ui.Device.browser.msie || sap.ui.Device.browser.edge)
			) {
		...
		(function ($, undefined) {
			...
			function transform3dTest() {
				var mqProp = "transform-3d",
					...
					ret = $.mobile.media(
						"(-" + vendors.join("-" + mqProp + "),(-") + "-" + mqProp + "),(" + mqProp + ")"
					);
			...
			$.mobile.gradeA = function () {
				return (
					($.support.mediaquery || ($.mobile.browser.oldIE && $.mobile.browser.oldIE >= 7)) &&
			...
			$.event.special.tap = {
				...
				setup: function () {
					...
					function clearTapHandlers() {
						...
						// MODIFIED BY SAP: replace deprecated API .unbind -> .off
						$this.off("vclick", clickHandler).off("vmouseup", clearTapTimer);
						$document
							.off("vmousecancel", clearTapHandlers)
					...
					function checkAndClearTapHandlers(mouseUpEvent) {
						...
						if (
							mouseUpEvent.target !== mouseDownTarget &&
							!$.contains(mouseDownTarget, mouseUpEvent.target) &&
							!buttonTappedInEdgeBrowser(mouseUpEvent)
						) {
							clearTapHandlers();
						}
					}
					...
					$this.on("vmousedown", function (event) {
						...
						// MODIFIED BY SAP: replace deprecated API .bind -> .on
						$this.on("vmouseup", clearTapTimer).on("vclick", clickHandler);
						$document
							.on("vmousecancel", clearTapHandlers)
				...
				setup: function () {
					...
					$this.on(touchStartEvent, function (event) {
						...
						function stopHandler(event) {
							// MODIFIED BY SAP: replace deprecated API .unbind -> .off
							$this.off(touchMoveEvent, moveHandler).off(touchStopEvent, stopHandler);
						...
						// MODIFIED BY SAP: replace deprecated API .bind -> .on
						$this.on(touchMoveEvent, moveHandler).on(touchStopEvent, stopHandler);
					});
				}
			};
			$.each(
