		...
		services: {
					...
					"config": {
						"groups": [{
							"tiles": [{
								...
								"properties": {
									<% if (applicationTitle) { %>"title": "<%= applicationTitle %>",<% } %>
									"targetURL": "#<%= navigationIntent %>-display"
								}
							}]
						}]
					}
			...
			"ClientSideTargetResolution": {
				"adapter": {
					"config": {
						"inbounds": {
							"<%= navigationIntent %>-display": {
								"semanticObject": "<%= navigationIntent %>",
								"action": "display",
								<% if (applicationDescription) { %>"description": "<%= applicationDescription %>",<% } %><% if (applicationTitle) { %>
