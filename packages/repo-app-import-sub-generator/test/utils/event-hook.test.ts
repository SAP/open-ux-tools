...
describe('runPostAppGenHook', () => {
    ...
    it('should log an error if vscodeInstance is missing', async () => {
        ...
        expect(RepoAppDownloadLogger.logger.error).toHaveBeenCalledWith(t('error.eventHookErrors.vscodeInstanceMissing'));
    });

    it('should log an error if postGenCommand is missing', async () => {
        mockContext.postGenCommand = '';
        await runPostAppGenHook(mockContext);
        expect(RepoAppDownloadLogger.logger.error).toHaveBeenCalledWith(t('error.eventHookErrors.postGenCommandMissing'));
    });
    ...
    it('should log an error if executeCommand throws an error', async () => {
        ...
        await runPostAppGenHook(mockContext);
        expect(RepoAppDownloadLogger.logger.error).toHaveBeenCalledWith(t('error.eventHookErrors.commandExecutionFailed'));
    });
});
