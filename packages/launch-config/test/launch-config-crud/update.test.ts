import { DirName, createDirectory, deleteDirectory, readFile } from '@sap-ux/project-access';
...
beforeAll(async () => {
    ...
    // copyFileSync(TestPaths.freestyleProjectsLaunchConfig, freestyleLaunchJsonCopy);
});

afterEach(async () => {
    const launchJsonString = await readFile(feLaunchJsonCopy);
...
test('Add launch config', async (): Promise<void> => {
    ...
    const launchConfigs = await getAllLaunchConfigs(TestPaths.tmpDir);
...
test('Update added launch config', async (): Promise<void> => {
    ...
    const changedConfigIndex = launchConfigs[0].launchConfigs.findIndex((c) => c.name === 'Changed config during test');
...
test('Update existing launch config with comments', async (): Promise<void> => {
    ...
    launchConfigs = await getAllLaunchConfigs(TestPaths.tmpDir);
    const changedConfigIndex = launchConfigs[0].launchConfigs.findIndex((c) => c.name === 'Existing config changed');
...
test('Delete added launch config', async (): Promise<void> => {
    let launchConfigs = await getAllLaunchConfigs(TestPaths.tmpDir);
    let addedConfigIndex = launchConfigs[0].launchConfigs.findIndex((c) => c.name === 'Changed config during test 3');
