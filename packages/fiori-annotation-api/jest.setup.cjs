...
const { symlink, rm, lstat } = require('fs/promises');
...
function npmInstall(projectPath) {
    ...
    const npm = spawnSync(cmd, ['install', '--ignore-engines'], {
        cwd: projectPath, env: process.env, shell: true, stdio: 'inherit', timeout: fiveMinutes
    });
    if (npm.error) console.log('Error: ' + npm.error.message);
    else if (npm.status !== 0) console.log('npm exited with code ' + npm.status);
    else console.log('Installed in ' + projectPath);
}
module.exports = async function () {
    ...
    for (const projectPath of CDS_PROJECTS) {
        ...
        try { await lstat(targetPath); await rm(targetPath, { recursive: true }); }
        catch (e) { /* target does not exist, ok */ }
...
function getCdsRoot(branch) {
    if (!branch) return CDS_LATEST_ROOT;
    if (branch === 'next') return CDS_NEXT_ROOT;
    if (branch === 'latest') return CDS_LATEST_ROOT;
    if (branch === 'maintenance') return CDS_MAINTENANCE_ROOT;
