...
entity Incidents : managed, identified {
  ...
  description             : String(1000)                      @Common.ValueList #propertyPropagation: { 
...
extend Incidents with {
  @Common.ValueList #structProp : { $Type : 'Common.ValueListType', CollectionPath : 'test' }
  extra : StuctType1
};

type StuctType1 {
  @Common.Label #collectionLabel : 'Collection Data' collection : CollectionType;
  publisherID : Integer;
}

type CollectionType {
  collectionName   : String(20);
  @Common.Label #collectionIDLabel : 'ID' collectionID : Integer;
}

annotate Incidents with @UI.HeaderInfo #incidentHeader : {
  $Type : 'UI.HeaderInfoType',
  TypeName : '',
  TypeNamePlural : '',
};

annotate Incidents with @Core.Description #incidentCoreDescOnDb : 'my DB description for incident';


annotate Incidents with @UI : {
  DataPoint #incidentResolveTime: {         // needed for test 1
    $Type : 'UI.DataPointType',
    Value : timeForResolutiion,
  },
};