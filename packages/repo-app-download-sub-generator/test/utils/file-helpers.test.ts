...
describe('readManifest', () => {
    ...
    it('should throw an error if manifest is not found', async () => {
        ...
        expect(BspAppDownloadLogger.logger.error).toBeCalledWith(t('error.readManifestErrors.readManifestFailed'));
    });
    ...
    it('should throw an error if "sap.app" is not defined in the manifest', async () => {
        ...
        readManifest(extractedProjectPath, mockFs)
        expect(BspAppDownloadLogger.logger.error).toBeCalledWith(t('error.readManifestErrors.sapAppNotDefined'));
    });
    ...
    it('should throw an error if the sourceTemplate.id is not supported', async () => {
        ...
        readManifest(extractedProjectPath, mockFs)
        expect(BspAppDownloadLogger.logger.error).toBeCalledWith(t('error.readManifestErrors.sourceTemplateNotSupported'));
    });
});
