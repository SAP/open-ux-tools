...
export class Ui5AbapRepositoryService extends ODataService {
    ...
    protected async updateRepoRequest(
        ...
        try {
            ...
            if (tryCount === 2) {
                this.log.warn(
                    'Warning: The BSP application deployment timed out while waiting for a response from the backend. This may indicate that the deployment did not finish completely. To resolve this, consider increasing the value of the HTML5.Timeout property for the destination.'
                );
            }
