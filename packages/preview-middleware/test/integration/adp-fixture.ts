...
export const test = base.extend<TestOptions, WorkerFixtures>({
    ...
    project: [
        async ({ ui5Version, projectConfig, backend }, use, testInfo) => {
            console.log(
            ...
            if (projectConfig.type === 'generated') {
                if (projectConfig.kind === 'adp') {
                    await generateUi5Project(projectConfig.baseApp, testInfo.parallelIndex.toString(), ui5Version);
                    const root = await generateAdpProject(
                        projectConfig,
                        testInfo.parallelIndex.toString(),
                        ui5Version,
                        backend
                    );
                    ...
                    const server = await setup({
                        command: `cd ${root} && npx ui5 serve --port=${port}`,
                    ...
                    console.log(`Stopping server for ${root}`);
    ...
    page: async ({ page, project }, use) => {
        await page.goto(
            `http://localhost:${project.port}${ADAPTATION_EDITOR_PATH}?fiori-tools-rta-mode=true#app-preview`
        );
