...
const checkTestsuite = async (param: { page: Page }) => {
    const { page } = param;
    const htmlResponse = await page.request.get(`${getUrl()}/test-resources/my/fe/v2/app/testsuite.qunit.html`);
    expect(htmlResponse.status()).toBe(200);
    const jsResponse = await page.request.get(`${getUrl()}/test-resources/my/fe/v2/app/testsuite.qunit.js`);
