...
| `flp.libs`              | `boolean`  | optional                                       | `true` (for EDMX backends)      | Flag to automatically fetch paths of libraries and components (from EDMX backend) that are not available in the UI5 sources. This avoids trying to load libraries and components from the `/resources` path although they are not part of UI5. |
