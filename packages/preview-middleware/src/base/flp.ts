...
export class FlpSandbox {
     ...
     *
     * @param config - the template config to mutate
    ...
    private async generateSandboxForEditor(req: EnhancedRequest, rta: RtaConfig, editor: RtaEditor): Promise<string> {
        ...
        const patchedRouterBaseUrl = (this.templateConfig.baseUrl = req['ui5-patched-router']?.baseUrl ?? '');
    ...
    private async flpGetHandler(
        ...
        if (file) {
            ...
            const html = render(await this.getSandboxTemplate(ui5Version), { ...this.templateConfig, baseUrl });
            this.sendResponse(res, 'text/html', 200, html);
        }
    }
