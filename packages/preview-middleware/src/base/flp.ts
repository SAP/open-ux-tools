...
export class FlpSandbox {
    ...
    private async editorGetHandler(
        ...
        if (!req.query['fiori-tools-rta-mode']) {
            ...
            res.redirect(302, `${url}?${new URLSearchParams(params as Record<string, string>)}`); // CodeQL[js/server-side-unvalidated-url-redirection] url is req.path (relative), not an external URL
            return;
        }
