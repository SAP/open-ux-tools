...
describe('processAiResponse', () => {
    beforeEach(() => {
        jest.clearAllMocks();
        mockWriteExtractedFile.mockReturnValue('webapp/changes/coding/MyExt.js');
    });

    test('returns Success with changes list when files are written', () => {
    ...
    test('returns Skipped when aiResponse contains no extractable files', () => {
        const result = processAiResponse(APP_PATH, 'just prose');
    ...
    test('skips .change files and does not call writeExtractedFile for them', () => {
        const mixedResponse = [
        ...
        const result = processAiResponse(APP_PATH, mixedResponse);
        expect(result.status).toBe('Success');
