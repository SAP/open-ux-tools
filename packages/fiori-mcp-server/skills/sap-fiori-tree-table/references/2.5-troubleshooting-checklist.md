...
**Why**: Directory filter isolates hierarchies by root entity (prevents mixing data from different roots)
...
**Why**: Hierarchy definition's `directory` clause requires this association for filtering
...
**Why**: Without this facet, Fiori Elements doesn't know to display the child tree table
...
**Rule**: Never proceed to next step until previous step's objects are successfully activated
