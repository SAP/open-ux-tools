...
- Package created (local `$TMP` or transportable)
  - **Local packages** (e.g., `$TMP`, `Z<namespace>`, `TEST<namespace>`): No transport request required
...
- **ADT MCP Tools activated**: Run `activate_abap_object_management_tools()` at the start of your session to enable write operations
  ...
  - Without activation, MCP tools operate in read-only mode

## Core Architecture Pattern

**When using RAP Generator (Option 2):**
...
**When creating from scratch (Option 3):**
Follow SAP's recommended 5-layer architecture:

1. **Persistent Layer**: Active and draft tables with UUID keys
2. **Interface Layer**: Basic CDS views with associations (e.g., `ZI_*`)
3. **Hierarchy Definition**: CDS hierarchy with directory filter
4. **Base View Layer**: Transactional views (e.g., `ZR_*`) with behavior
5. **Projection Layer**: Consumption views (e.g., `ZC_*`) with OData hierarchy annotation
...
**Starting**: "I've analyzed your requirements and created a detailed implementation plan. This will involve 8 sequential steps. Would you like to review the plan first, or shall we proceed with Step 1?"
...
**Completion**: "🎉 All 8 steps completed! Service is published and ready for testing. Would you like to: 1) create SAP Fiori elements application, 2) Create test data, or 3) Review the implementation?"
