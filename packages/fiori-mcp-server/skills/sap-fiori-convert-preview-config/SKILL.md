---
name: sap-fiori-convert-preview-config
description: Converts a SAP Fiori app's preview configuration from local HTML files to virtual endpoints using `npx @sap-ux/create@latest convert preview-config`. Use this skill whenever the user wants to migrate or convert a Fiori app preview, run the preview-config conversion, upgrade from local sandbox HTML to virtual preview endpoints, or asks about converting preview files. After running the command, detects manual migration steps in the output (Karma config, WebdriverIO QUnit Service paths) and uses AI to complete them automatically.
metadata:
...
Runs `npx --yes @sap-ux/create@latest convert preview-config` on a SAP Fiori app, then detects and automatically handles any manual migration steps that the tool cannot do itself.

## What the command does

- Reads `package.json` scripts to determine the preview configuration
- Updates `ui5.yaml` / `ui5-local.yaml` for the new virtual preview middleware
- Renames old HTML files to `*_old.html` (they are no longer needed)
- Deletes obsolete JS/TS preview source files
- Optionally converts test runners (`--tests=true`)
...
```bash
npx --yes @sap-ux/create@latest convert preview-config [path] --simulate=<true|false> --tests=<true|false>
```
...
3. Create `webapp/test/flpSandboxInit.js` (or `.ts`) with the extracted code:
   - For `.ts`: the code can be placed directly (it is executed after standard FLP init, so no wrapper needed)
   - For `.js`: same - place the extracted code directly, no wrapper needed
   ```ts
...
6. Update `ui5.testpage` to the new virtual endpoint path if it changed
