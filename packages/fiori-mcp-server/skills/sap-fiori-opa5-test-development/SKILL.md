...
description: Use this skill whenever writing, fixing, extending, or reviewing OPA5 integration tests for SAP Fiori elements (OData V4) applications. Triggers on any of "write OPA5 test for Fiori elements", "add OPA test to list report", "add OPA test to object page", "test object page with OPA", "sap.fe.test", "JourneyRunner", or any request involving integration testing of a Fiori Elements app generated by SAP Fiori tools. Use even if the user just says "add an OPA journey" or "write integration tests" in the context of a SAP Fiori elements project.
metadata:
...
This skill requires an existing SAP Fiori Elements for OData V4 application generated by SAP Fiori tools. The `/test` folder must be present (e.g. at `/webapp/test`). If it is missing, trigger regeneration first (see "Generated Test Structure" below).
...
Implication for test development: When working in a virtual-endpoint project, do not create `opaTests.qunit.html` or `OpaTests.qunit.js` manually. New journey files are picked up automatically as long as their filename matches the configured pattern (default: ends in Journey.js or Journey.ts).
...
The SAP Fiori Elements mock server (`@sap-ux/ui5-middleware-fe-mockserver`) runs entirely in the UI5 tooling layer - no backend process needed. This is the recommended approach for OPA5 tests because it keeps tests isolated to the UI, fast, and reproducible.
...
For CAP-based projects the CAP server (`cds watch` or `npm start`) can serve as the data backend for OPA5 tests. The tests run against the real OData service, so no mock data files are needed on application level.
