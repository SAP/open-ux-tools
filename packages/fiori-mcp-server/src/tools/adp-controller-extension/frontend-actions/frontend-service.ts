 ...
 *
 * Honors the `PLAYWRIGHT_CHROMIUM_EXECUTABLE_PATH` environment variable (a
 * variable defined by this module, not Playwright itself) to point at a
 * system Chrome/Chromium binary. Otherwise relies on Playwright's own
 * binary cache.
 */
...
export async function callFrontendAction<TReturn = unknown>(
    ...
    if (rpc) {
        try {
            // Cheap liveness probe: if the bridge isn't available the page is
            // still good for a goto+retry, but if evaluate throws the page is
            // gone.
            await rpc.callFrontendAction('__liveness__', {});
        } catch (error) {
            // The page may legitimately throw "action not registered" — that's
            // fine and means the page is alive. Anything else (page closed,
            // navigation away, etc.) means we evict.
            const message = error instanceof Error ? error.message : String(error);
            if (!message.includes('not registered') && !message.includes('not available')) {
