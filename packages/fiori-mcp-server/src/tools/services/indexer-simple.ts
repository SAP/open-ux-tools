...
export class SimpleDocumentIndexer {
     ...
     */
    constructor(
        docsPath: string = '../data/docs',
        embeddingsPath: string = '../data/embeddings',
        ...
        try {
            ...
            if (this.vectorService) {
                try {
                    ...
                    logger.warn('Vector service initialization failed, disabling vector search:', error);
    ...
    private async loadKeywordIndex(): Promise<void> {
        try {
            const keywordIndexPath = path.join(__dirname, '../data/search', 'keywords.json');
            ...
            logger.log(`✓ Loaded keyword index: ${this.keywordIndex.size} terms`);
        } catch (error) {
